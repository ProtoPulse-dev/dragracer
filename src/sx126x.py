# SPDX-FileCopyrightText: 2020 E H Ong
# SPDX-FileCopyrightText: 2018 Jan Gromeš
#
# SPDX-License-Identifier: MIT
#
# This file is derived from the micropySX126X project:
#   https://github.com/ehong-tl/micropySX126X
# The original project is ported from RadioLib by Jan Gromeš.
# License text: licenses/micropySX126X-LICENSE.txt
#
# Local modifications for the Heltec Wireless Tracker V2:
#   - Added SoftSPI support (spi_bus=-1 path).
#   - Reworked SPItransfer() to use single-byte full-duplex transfers because
...
class SX126X:
    ...
    def __init__(self, spi_bus, clk, mosi, miso, cs, irq, rst, gpio):
        ...
        if implementation.name == 'micropython':
          if spi_bus == -1:
              # Use bit-banged SoftSPI.  Needed on the Heltec Wireless Tracker
              # V2 because the SX1262 is on GPIO 8/9/10/11 which is not a
              # hardware SPI channel exposed by MicroPython.
              self.spi = SoftSPI(baudrate=1_000_000, polarity=0, phase=0,
    ...
    def SPItransfer(self, cmd, cmdLen, write, dataOut, dataIn, numBytes, waitForBusy, timeout=5000):
        # SX1262 SPI protocol: BUSY must be low before asserting CS, CS must
        # stay low for the entire transaction, and the chip returns a status
        # byte on MISO during the first byte after the command.  MicroPython
        # SoftSPI's spi.read() does not honour the out-going byte, so we use
        # single-byte write_readinto() calls to keep true full-duplex timing.
        if implementation.name == 'micropython':
            start = ticks_ms()
            while self.gpio.value():
                yield_()
            ...
            # Command bytes (ignore MISO during this phase).
            ...
            if write:
                for i in range(numBytes):
                    try:
                        ...
                        # Old SoftSPI without write_readinto: fall back to
                        # read with the data byte as the outgoing pattern.
                        in_[0] = self.spi.read(1, write=dataOut[i])[0]
                ...
                # First byte after command is a dummy that returns status.
                try:
                    self.spi.write_readinto(bytes([SX126X_CMD_NOP]), in_)
                except AttributeError:
                    in_[0] = self.spi.read(1, write=SX126X_CMD_NOP)[0]
                ...
                if (in_[0] & 0b00001110) == SX126X_STATUS_CMD_TIMEOUT or\
                   ...
                   (in_[0] & 0b00001110) == SX126X_STATUS_CMD_FAILED:
                    ...
                    # Status byte is valid; now read numBytes response bytes.
                    for i in range(numBytes):
                        try:
                            self.spi.write_readinto(bytes([SX126X_CMD_NOP]), in_)
                        except AttributeError:
