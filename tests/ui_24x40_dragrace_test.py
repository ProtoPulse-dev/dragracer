"""
Drag-race context UI test with 24x40 font.
Initialises the same hardware/services as drag_race.py and shows the new UI
for a few seconds. Safe to run with `mpremote run`.
"""
import machine
import time
import network
...
def update_tft():
    ...
    if current_screen == 0:
        ...
        if state == "RUN":
            ...
            digits24x40.draw_number_trimmed(disp, timer_str, 0, 14, RED)
            text8("s", 132, 52, GRAY, BLACK)
            text8("%5.1f kmh" % current_speed, 80, 70, WHITE, BLACK)
            span = max(1.0, _CFG["end"] - _CFG["start"])
            pct = (current_speed - _CFG["start"]) / span
            _draw_progress_bar(0, 63, 160, 3, pct, RED)
        else:
            speed_str = "%5.1f" % current_speed
            digits24x40.draw_number_trimmed(disp, speed_str, 0, 14, WHITE)
            text8("km/h", 132, 45, GRAY, BLACK)
            timer_str = "%5.1f" % current_timer
            text8("T" + timer_str, 0, 70, DARK_GRAY, BLACK)
    else:
...
# Also start the webserver to stress memory similarly to drag_race.py.
ap = network.WLAN(network.AP_IF)
ap.config(essid="GPS_TIMER_ESP32", password="12345678")
ap.active(True)
