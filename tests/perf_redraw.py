"""
perf_redraw.py — measure how long one new-UI frame takes to render and
whether continuous redraw at low cadence remains stable.
"""
import machine
import time
...
from heltec_tracker_v2 import (
    ...
    VEXT_CTRL, GNSS_RST, GNSS_TX, GNSS_RX, GNSS_BAUD,
)
...
# Render READY screen once for timing
t0 = time.ticks_ms()
disp.fill(BLACK)
...
disp.show()
t1 = time.ticks_ms()
print("one READY frame ms:", time.ticks_diff(t1, t0))

# Render RUN screen once for timing
t0 = time.ticks_ms()
disp.fill(BLACK)
...
disp.show()
t1 = time.ticks_ms()
print("one RUN frame ms:", time.ticks_diff(t1, t0))

# Continuous redraw at 2 Hz for 5 seconds
print("2 Hz redraw for 5s...")
t0 = time.ticks_ms()
count = 0
state = "WAI"
while time.ticks_diff(time.ticks_ms(), t0) < 5000:
    now = time.ticks_ms()
    if time.ticks_diff(now, t0) // 500 != count:
        count = time.ticks_diff(now, t0) // 500
        state = "RUN" if (count % 2) else "WAI"
        disp.fill(BLACK)
        disp.fill_rect(0, 0, 4, 8, GREEN if state == "WAI" else RED)
        text8(state, 8, 0, GREEN if state == "WAI" else RED)
        text8("S8", 142, 0, GRAY)
        disp.hline(0, 9, 160, DARK_GRAY)
        if state == "RUN":
            digits24x40.draw_number_trimmed(disp, "3.85", 0, 14, RED)
            text8("s", 132, 52, GRAY)
            text8("97.3 kmh", 80, 70, WHITE)
            _draw_progress_bar(0, 63, 160, 3, 0.6, RED)
        else:
            digits24x40.draw_number_trimmed(disp, "  0.0", 0, 14, WHITE)
            text8("km/h", 132, 45, GRAY)
            text8("T0.0", 0, 70, DARK_GRAY)
...
print("2 Hz redraw done, frames:", count)
print("alive")
